...
float max_moove_force = 25.0f;
float min_moove_force = 0.1f;

float palyer_moove_speed = 1.0f;
...
{
	//Create the window
	sf::RenderWindow render_window(sf::VideoMode(2000, 600), "Le Gayme");
	...
	while (render_window.isOpen())
	{
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::A))
		{
			...
			move_force = sf::Vector2f(palyer_moove_speed * -1, 0.0f);
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))
		{
			//Move the player right
			move_force = sf::Vector2f(palyer_moove_speed, 0.0f);
		}
		else
		{
		...
		//Cap the min Speed
		if (std::abs(speed.x) < min_moove_force)
		{
			speed.x = 0.0f;
		}

		//Cap the max Speed
		if (speed.x > max_moove_force)
		{
			speed.x = max_moove_force;
		}
		else if (speed.x < max_moove_force * -1)
		{
			speed.x = max_moove_force * -1;
		}
